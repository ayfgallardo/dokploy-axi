...
- Token accounting lives in `src/gain.ts`: `request()` in `src/dokploy.ts` is the single point every response passes through, and it records only the body it keeps — never the 500 the tRPC fallback retried. Counting must stay there and never move into `send()`: up to ~46 requests of one invocation are in flight at once (`home` fans out over every service), so anything that assumes the last body recorded belongs to the call in hand is wrong. `gpt-tokenizer` is imported dynamically after stdout is written — never move it to a static import, it would load its BPE tables on every invocation. `AXI_GAIN=0` disables the whole path.

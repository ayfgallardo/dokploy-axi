...
/**
 * Only names the CLI itself defines are ever recorded, never user input. Both
 * tokens of a `service view` style name come from closed lists, so a service
 * name or a flag value can never reach the log.
 */
export function gainCommandName(
  ...
  if (sub !== undefined && second !== undefined && sub.includes(second)) {
    return `${first} ${second}`;
  }
  return first;
}
